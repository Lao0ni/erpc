...
RUN --mount=type=cache,id=cache-pnpm,target=/pnpm/store cd /temp/dev &&  pnpm install --frozen-lockfile 
...
RUN --mount=type=cache,id=cache-pnpm,target=/pnpm/store cd /temp/prod && pnpm install --prod --frozen-lockfile
